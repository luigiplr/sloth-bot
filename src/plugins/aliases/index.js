...
export function alias(sender, channel, input, ts, plugins, userLevel) {
  return new Promise((resolve, reject) => {
    ...
    const usernameIsNotName = username.match(/^(set|view|del|delete|remove)$/)
    ...
    let user = findUser(usernameIsNotName ? sender.id : username)
    ...
    switch (method) {
      case 'set':
        if (!isSender && userLevel !== 'superadmin') return reject("You can't edit other peoples aliases")
      ...
      case 'del':
      case 'remove':
        if (!isSender && userLevel !== 'superadmin') return reject("You can't delete other peoples aliases")
        if (!service) return reject("You need to specify a service (steam, overwatch, etc..)")
      ...
      case 'view':
        _getUserAliases(user.id).then(data => {
          if (!data.length) return reject(`${isSender ? 'You have' : 'User has'} no aliases set`)
