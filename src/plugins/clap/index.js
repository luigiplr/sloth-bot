...
module.exports = {
  ...
  clap(user, channel, input = '') {
    return new Promise(resolve => {
      let type = input.split(' ')[0] || undefined;
      let clapee = input.split(' ')[1] || undefined;
      ...
      switch (type) {
        ...
        case 'bravo':
          resolve({
            type: 'channel',
            message: (clapee ? '(' + clapee + ') ' : '') + images.insincere[Math.floor(Math.random() * images.insincere.length)]
          });
          break;
        default:
          clapee = type;
          resolve({
            type: 'channel',
            message: (clapee ? '(' + clapee + ') ' : '') + images.sincere[Math.floor(Math.random() * images.sincere.length)]
          });
      }
    });
  }
