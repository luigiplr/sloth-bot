...
var queryTorrents = (query, retry) => {
    return new Promise((resolve, reject) => {
        ...
        if (!query || (query && typeof query !== 'string' && !query.query && !query.category && !query.min_seeds && !query.uploader && !query.age && !query.safety_filter && !query.verified && !query.language)) {
            return reject('Missing a mandatory parameter');
        }
        ...
        request(requestUri.toString(), {
            ...
            if (error) {
                ...
                reject(new Error('No data'), retry);
            } else if (!body.list || Object.keys(body.list).length === 0) {
                reject(new Error('No results'), retry);
            } else {
                resolve(format(body, query.page || 1, Date.now() - t));
            }
        });
    });
};

var format = (response, page, responseTime) => {
    var formatted = {};
